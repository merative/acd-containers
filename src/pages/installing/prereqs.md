...
For high availability it is recommended to run 3 replicas of ACD and its micro-services on a minimum of 3 worker nodes that have 16 vCPU/node and 64 GB of memory. For a development or test environment, 1 or 2 replicas can be configured and 8 vCPU/pod can be used.  

By default the ACD pods are allowed to use all of the CPUs on a node. If needed you can limit the ACD deployment CPU usage (see [Configuration](./README.md#configuration)), but this may result in lower availability of the ACD service.

These are the requirements for ACD. The cluster itself has additional minimal worker node requirements for monitoring,
