...
Similar to the [concept detection](/clouddocs/annotator_concept_detection/) annotator, the attribute detection annotator may attach the medical codes for applicable concepts; e.g. NCI, ICD-9, ICD-10, LOINC, MeSH, RxNorm, SNOMED CT, and CPT codes. Attribute detection can also provide two additional medical codes (CCS code and HCC Code) made available by the [cancer](/clouddocs/annotator_cancer/) and [symptom disease](/clouddocs/annotator_symptom_disease/) annotators. The consumers can elect to have the set of medical codes associated with the attribute by specifying the optional configuration parameter to return the medical codes.
